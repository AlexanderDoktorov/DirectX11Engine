...
public:
	//																				   theta       phi
	static IndexedTriangleList MakeTesselated2( DynamicVertex::VertexLayout layout, int latDiv, int longDiv )
	{
		...
		DynamicVertex::VertexBuffer vb{ std::move( layout ), static_cast<size_t>(2 * longDiv * (latDiv - 1))};
	...
	static IndexedTriangleList MakeTessaletedNormalized(DynamicVertex::VertexLayout layout, int numSlices, int numStacks, float r = 1.f)
	{
		// numverts = 2 * numSlices * numStacks
	...
	static IndexedTriangleList MakeTesselated(DynamicVertex::VertexLayout layout, int numSlices, int numStacks, float radius = 1.f)
	{
		using enum DynamicVertex::VertexLayout::ElementType;
		DynamicVertex::VertexBuffer vb{ std::move( layout ), (size_t)(numSlices + 1) * (numStacks + 1) };
		std::vector<unsigned short> indices;

		for (int i = 0; i <= numStacks; ++i) // numStacks + 1 * numSlices + 1
		{
			float phi = DirectX::XM_PI * static_cast<float>(i) / static_cast<float>(numStacks);
			for (int j = 0; j <= numSlices; ++j)
			{
				...
				int vertexIndex = i * (numSlices + 1) + j;
				vb[vertexIndex].Attr<Position3D>() = pos;
			}
		}
	...
	static IndexedTriangleList MakeTesselatedTextured(DynamicVertex::VertexLayout layout, int numSlices, int numStacks, float radius = 1.f)
	{
		using enum DynamicVertex::VertexLayout::ElementType;
		auto itl = MakeTesselated(layout, numSlices, numStacks, radius);
		...
		for (int i = 0; i <= numStacks; ++i)
		{
			for (int j = 0; j <= numSlices; ++j)
			{
				...
				itl.vertices[vertexIndex].Attr<Texture2D>() = dx::XMFLOAT2(tcx, tcy);
			}
		}
	...
	static IndexedTriangleList MakeTesselatedTexturedNormalized(DynamicVertex::VertexLayout layout, int numSlices, int numStacks, float r = 1.f)
	{
		using enum DynamicVertex::VertexLayout::ElementType;
		DynamicVertex::VertexBuffer vb{ std::move( layout ) };
		std::vector<unsigned short> indices;
